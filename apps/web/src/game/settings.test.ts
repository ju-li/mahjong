...
describe('settings', () => {
  it('falls back to defaults without storage', async () => {
    ...
    expect(load()).toEqual({ claimSeconds: 10, sound: true, difficulty: 'medium', rules: 'mcr', textSize: 'normal' })
  })
  ...
  it('writes every setting to localStorage and reads it back', async () => {
    ...
    await nextTick()
    expect(JSON.parse(data.get('mahjong.settings.v1')!)).toEqual({ claimSeconds: 5, sound: false, difficulty: 'hard', rules: 'hk', textSize: 'larger' })

    const reloaded = await importSettings()
    expect(reloaded.load()).toEqual({ claimSeconds: 5, sound: false, difficulty: 'hard', rules: 'hk', textSize: 'larger' })
  })
  ...
  it('takes difficulty and rules from a match saved by an older version', async () => {
    ...
    const { load } = await importSettings()
    expect(load()).toEqual({ claimSeconds: 20, sound: true, difficulty: 'easy', rules: 'hk', textSize: 'normal' })
  })
