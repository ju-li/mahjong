...
import { FANS, type FanId } from '@mahjong/engine'
...
const groups = computed(() => {
  ...
  const match = (f: (typeof FANS)[number]) =>
    !q ||
    [f.name, f.chinese, f.description.en, f.description.zh].some((text) => text.toLowerCase().includes(q)) ||
    String(f.points) === q
  const byPoints = new Map<number, (typeof FANS)[number][]>()
  for (const f of FANS.filter(match)) byPoints.set(f.points, [...(byPoints.get(f.points) ?? []), f])
  return [...byPoints].sort((a, b) => b[0] - a[0])
})

const otherName = (id: FanId) => {
