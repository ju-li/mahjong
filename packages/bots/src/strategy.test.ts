...
/** Points `stronger` nets over 30 seeded hands against `weaker`, seats alternating. */
function netScore(stronger: Difficulty, weaker: Difficulty): number {
  let net = 0
  for (let seed = 1; seed <= 30; seed++) {
    const levels: Difficulty[] = seed % 2 ? [weaker, stronger, weaker, stronger] : [stronger, weaker, stronger, weaker]
    ...
    if (r?.type === 'win') r.deltas.forEach((d, s) => levels[s] === stronger && (net += d))
  }
  return net
}
...
describe('bot strength', () => {
  it('medium outscores easy over 30 seeded hands (seats alternate)', () => {
    expect(netScore('medium', 'easy')).toBeGreaterThan(0)
  })

  it('easy outscores beginner over 30 seeded hands (seats alternate)', () => {
    expect(netScore('easy', 'beginner')).toBeGreaterThan(0)
  })
})
